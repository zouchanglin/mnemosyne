        ...
        <van-row>
            <van-list
                ...
                <van-swipe-cell v-for="(item, index) in list" :key="index" style="height: 100%">
                    <template #left>
                        <van-button square type="default" text="未学过" style="height: 100%"
                                    @click="add_learn_word(item.id, false)"/>
                        <van-button square type="warning" text="已忘记" style="height: 100%"
                                    @click="add_killed_word(item.id, false)"/>
                    </template>
                    <van-cell :border="true" clickable="clickable" @click="click_word(item.word)">
                        <template #title>
                        ...
                        <template #right-icon>
                            <van-tag plain type="primary" v-for="(tag, index) in item.tags"
                                     size="medium"
                                     :key="index" style="margin-right: 5px">{{ tag }}
                            </van-tag>
                        </template>
                    </van-cell>
                    <template #right>
                        <van-button square type="info" text="学过" style="height: 100%"
                                    @click="add_learn_word(item.id, true)"/>
                        <van-button square type="primary" text="熟练" style="height: 100%"
                                    @click="add_killed_word(item.id, true)"/>
                    </template>
                </van-swipe-cell>
            </van-list>
        </van-row>
